...
{
	if (!strcmp(paramName,"activeVal"))		m_configData->activeVal		= atof(paramValue);
	if (!strcmp(paramName,"inactive"))		m_configData->inactive		= atoi(paramValue);
	if (!strcmp(paramName,"inactiveVal"))	m_configData->inactiveVal	= atof(paramValue);
	if (!strcmp(paramName,"minimized"))		m_configData->minimized		= atoi(paramValue);
	if (!strcmp(paramName,"minimizedVal"))	m_configData->minimizedVal	= atof(paramValue);
}
...
char *Cmod_fpsApp::saveConfigParam(char *paramName)
{
	...
	if (!strcmp(paramName,"activeVal"))		sprintf(buf,"%.2f",m_configData->activeVal);
	if (!strcmp(paramName,"inactive"))		sprintf(buf,"%d",m_configData->inactive);
	if (!strcmp(paramName,"inactiveVal"))	sprintf(buf,"%.2f",m_configData->inactiveVal);
	if (!strcmp(paramName,"minimized"))		sprintf(buf,"%d",m_configData->minimized);
	if (!strcmp(paramName,"minimizedVal"))	sprintf(buf,"%.2f",m_configData->minimizedVal);
