...
DWORD CButtonST::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	...
	if (m_bIsPressed)
	{
		...
		{
			if (m_bDrawBorder)
				pDC->Draw3dRect(pRect, m_crColors[BTNST_COLOR_SHADOW], m_crColors[BTNST_COLOR_HILIGHT]);
		}
		else    
		{
	...
	else // ...else draw non pressed button
	{
		if (m_bIsFlat)
		{
			if (m_bMouseOnButton && m_bDrawBorder)
				pDC->Draw3dRect(pRect, m_crColors[BTNST_COLOR_HILIGHT], m_crColors[BTNST_COLOR_SHADOW]);
		}
		else
		{
